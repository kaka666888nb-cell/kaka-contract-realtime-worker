...
const FLOW_SCAN_BATCH_BY_PROVIDER = Object.freeze({
  // Step983: keep all 15 focus symbols active per provider, with a small
  // rotating tail so the legacy full-universe flow discovery is preserved.
  binance: Math.max(15, Math.min(BINANCE_FLOW_MAX_STATES - 2, Number(process.env.KAKA_FLOW_SCAN_BINANCE_BATCH || 18))),
  okx: Math.max(15, Number(process.env.KAKA_FLOW_SCAN_OKX_BATCH || 17)),
  bybit: Math.max(15, Number(process.env.KAKA_FLOW_SCAN_BYBIT_BATCH || 17)),
  bitget: Math.max(15, Number(process.env.KAKA_FLOW_SCAN_BITGET_BATCH || 17)),
  gate: Math.max(15, Number(process.env.KAKA_FLOW_SCAN_GATE_BATCH || 17)),
});
...
const sharedMetricRotationState = {
  ...
  meta_attempts: Object.fromEntries([...PROVIDERS].map((provider) => [provider, 0])),
  meta_successes: Object.fromEntries([...PROVIDERS].map((provider) => [provider, 0])),
  meta_errors: Object.fromEntries([...PROVIDERS].map((provider) => [provider, ''])),
  current_rows_persisted: 0,
  current_rows_physical_written: 0,
  current_rows_unchanged: 0,
  current_persist_error: '',
};
const flowScanState = {
  started: false,
  timer: null,
...
// Step770: persist the current funding/mark/index metadata collected by the same
// bounded backend rotation. The shared Data-page snapshot can then read OI,
// ratios, funding, mark/index and basis from Supabase without per-user exact calls.
const SHARED_CURRENT_META_TABLE = 'app_funding_rate_current_cache';
const SHARED_CURRENT_META_DIFF_RPC = 'app_upsert_funding_rate_current_batch_diff';
...
const BINANCE_OI_CRITICAL_DELAY_MS = 0;
...
let binanceOfficialTakerPartialCycles = 0;
let binanceOfficialTakerDebtRecoveries = 0;
...
async function loadPersistedHistory(state) {
  ...
  try {
    ...
    for (const raw of Array.isArray(rows) ? rows : []) {
      ...
      if (row) {
        state.completedBuckets.set(row.start, row);
        publishContractFlowHotScoreRows([row]);
      }
    }
...
function finalizeReadyBuckets(state, now = Date.now()) {
  ...
  for (const [start, bucket] of [...state.openBuckets.entries()]) {
    ...
    if (normalized) {
      state.completedBuckets.set(start, normalized);
      publishContractFlowHotScoreRows([normalized]);
    }
    queuePersist(row);
  }
  pruneState(state);
...
function focusPoolSymbolsByProvider() {
  const snapshot = getContractFocusPoolInternalSnapshot();
  const result = Object.fromEntries([...PROVIDERS].map((provider) => [provider, []]));
  if (snapshot?.ready !== true) return { ready: false, round: Number(snapshot?.round || 0), byProvider: result };
  for (const provider of PROVIDERS) {
    const rows = Array.isArray(snapshot?.providers?.[provider]?.rows) ? snapshot.providers[provider].rows : [];
    result[provider] = [...new Set(rows.map((row) => symbolKey(row?.symbol)).filter((symbol) => symbol && symbol.endsWith('USDT')))].slice(0, 15);
  }
  return { ready: [...PROVIDERS].every((provider) => result[provider].length === 15), round: Number(snapshot?.round || 0), byProvider: result };
}

function focusFlowBindingPayload() {
  ...
  let connectedRows = 0;
  for (const provider of PROVIDERS) {
    const symbols = focus.byProvider[provider] || [];
    targetByProvider[provider] = symbols.length;
    let active = 0;
    let connected = 0;
    for (const symbol of symbols) {
      const state = states.get(`${provider}:${symbol}`);
      if (state) active += 1;
      if (state?.ws?.readyState === WebSocket.OPEN) connected += 1;
    }
    activeByProvider[provider] = active;
    connectedByProvider[provider] = connected;
    targetRows += symbols.length;
    activeRows += active;
  ...
  return {
    ...
    target_by_provider: targetByProvider,
    active_by_provider: activeByProvider,
    connected_by_provider: connectedByProvider,
    target_rows: targetRows,
    active_rows: activeRows,
    connected_rows: connectedRows,
    all_focus_active: focus.ready && targetRows === 75 && activeRows === 75,
    all_focus_connected: focus.ready && targetRows === 75 && connectedRows === 75,
    extra_rotation_preserved: true,
    user_reads_open_focus_streams: false,
  };
}
...
function flowScanStatusPayload() {
  ...
  return {
    ...
    mode: 'focus_pool_15_each_plus_full_universe_rotating_tail_websocket',
...
function nextFlowScanBatch(provider, symbols) {
  ...
  const focus = focusPoolSymbolsByProvider();
  const focusSymbols = (focus.byProvider[provider] || [])
    .filter((symbol) => symbols.includes(symbol))
    .slice(0, Math.min(15, size));
  const selectedSet = new Set(focusSymbols);
  const pinned = SHARED_METRIC_ROTATION_PINNED
    .filter((symbol) => symbols.includes(symbol) && !selectedSet.has(symbol))
    .slice(0, Math.max(0, size - focusSymbols.length));
  for (const symbol of pinned) selectedSet.add(symbol);
  const rotatingSize = Math.max(0, size - selectedSet.size);
  ...
  return [...focusSymbols, ...pinned, ...rotating];
}
...
function sharedMetricRotationPayload() {
  return {
    ...
    current_meta_attempts_by_provider: sharedMetricRotationState.meta_attempts,
    current_meta_success_by_provider: sharedMetricRotationState.meta_successes,
    current_meta_errors_by_provider: sharedMetricRotationState.meta_errors,
    current_meta_storage_table: SHARED_CURRENT_META_TABLE,
    current_meta_persistence_rpc: SHARED_CURRENT_META_DIFF_RPC,
    current_meta_same_source_replay_noop: true,
    current_meta_freshness_advances_with_source_or_business_change: true,
    current_rows_persisted: sharedMetricRotationState.current_rows_persisted,
    current_rows_physical_written: sharedMetricRotationState.current_rows_physical_written,
    current_rows_unchanged: sharedMetricRotationState.current_rows_unchanged,
    current_persist_error: sharedMetricRotationState.current_persist_error,
    current_meta_uses_same_bounded_targets: true,
    current_meta_scales_with_users: false,
  };
}
...
function sharedCurrentMetaPersistRow(meta, provider, symbol) {
  if (!meta || typeof meta !== 'object' || meta.stale === true) return null;
  const funding = asNumber(meta.last_funding_rate ?? meta.funding_rate);
  const fundingPct = asNumber(meta.last_funding_rate_percent ?? meta.funding_rate_percent);
  const mark = asNumber(meta.mark_price);
  const index = asNumber(meta.index_price);
  const nextFunding = meta.next_funding_time ? String(meta.next_funding_time) : null;
  const hasFunding = funding != null && Number.isFinite(funding);
  const hasMark = mark != null && Number.isFinite(mark) && mark > 0;
  const hasIndex = index != null && Number.isFinite(index) && index > 0;
  if (!hasFunding && !hasMark && !hasIndex && !nextFunding) return null;
  return {
    provider,
    market_type: 'contract',
    symbol,
    mark_price: hasMark ? mark : null,
    index_price: hasIndex ? index : null,
    last_funding_rate: hasFunding ? funding : null,
    last_funding_rate_percent: fundingPct != null && Number.isFinite(fundingPct)
      ? fundingPct
      : (hasFunding ? funding * 100 : null),
    next_funding_time: nextFunding,
    source_time: meta.source_time || new Date().toISOString(),
    cached_at: new Date().toISOString(),
  };
}

async function persistSharedCurrentMetaRows(rows) {
  if (!PERSISTENCE_ENABLED || !rows.length) {
    return { logical: 0, written: 0, unchanged: 0 };
  }
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/rpc/${SHARED_CURRENT_META_DIFF_RPC}`,
    {
      method: 'POST',
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
  ...
  const text = await response.text();
  if (!response.ok) {
    throw new Error(`${SHARED_CURRENT_META_DIFF_RPC}_http_${response.status}:${text.slice(0, 220)}`);
  }
  let payload = null;
  try {
    payload = JSON.parse(text || '{}');
  ...
  if (
    ...
    throw new Error(
      `${SHARED_CURRENT_META_DIFF_RPC}_invalid_result:${String(text || '').slice(0, 260)}`,
    );
  }
  return { logical: rows.length, written, unchanged };
}
...
async function runSharedMetricRotationCycle() {
  ...
  sharedMetricRotationState.meta_attempts = Object.fromEntries([...PROVIDERS].map((provider) => [provider, 0]));
  sharedMetricRotationState.meta_successes = Object.fromEntries([...PROVIDERS].map((provider) => [provider, 0]));
  sharedMetricRotationState.meta_errors = Object.fromEntries([...PROVIDERS].map((provider) => [provider, '']));
  sharedMetricRotationState.current_rows_persisted = 0;
  sharedMetricRotationState.current_rows_physical_written = 0;
  sharedMetricRotationState.current_rows_unchanged = 0;
  sharedMetricRotationState.current_persist_error = '';
  sharedMetricRotationState.running = (async () => {
    await runBoundedConcurrency(
      ...
      async ({ provider, symbol }) => {
        ...
        sharedMetricRotationState.meta_attempts[provider] += 1;
        try {
          const meta = await getContractMeta(provider, symbol);
          const row = sharedCurrentMetaPersistRow(meta, provider, symbol);
          if (row) {
            currentMetaRows.set(`${provider}:${symbol}`, row);
            sharedMetricRotationState.meta_successes[provider] += 1;
          } else {
            sharedMetricRotationState.meta_errors[provider] = meta?.stale === true
              ? 'current_meta_stale_not_persisted'
              : 'current_meta_fields_empty';
          }
        } catch (error) {
          sharedMetricRotationState.meta_errors[provider] = String(error?.message || error).slice(0, 240);
        }
    ...
    await flushMetricPersistQueue();
    try {
      const persistResult = await persistSharedCurrentMetaRows(
        [...currentMetaRows.values()],
      );
      sharedMetricRotationState.current_rows_persisted = persistResult.logical;
      sharedMetricRotationState.current_rows_physical_written = persistResult.written;
      sharedMetricRotationState.current_rows_unchanged = persistResult.unchanged;
      sharedMetricRotationState.current_persist_error = '';
    } catch (error) {
      sharedMetricRotationState.current_persist_error = String(error?.message || error).slice(0, 300);
    }
...
async function runContractFlowUniverseScanCycle() {
  ...
  flowScanState.running = (async () => {
    ...
    await hydrateHotScoreFlowMetricsFromPersistence().catch(() => 0);
...
export async function reconcileContractFlowFocusPool() {
  return await runContractFlowUniverseScanCycle();
}
...
export function startContractFlowUniverseScanner() {
  ...
  const hotScoreHydrateTimer = setTimeout(() => hydrateHotScoreFlowMetricsFromPersistence({ force: true }).catch(() => 0), 900);
  hotScoreHydrateTimer.unref?.();
...
export function getContractFlowInternalFocusSnapshot() {
  const focus = focusPoolSymbolsByProvider();
  const rows = [];
  let activeRows = 0;
  let connectedRows = 0;
  for (const provider of PROVIDERS) {
    for (const symbol of focus.byProvider[provider] || []) {
      const state = states.get(`${provider}:${symbol}`);
      if (!state) continue;
      activeRows += 1;
      if (state.ws?.readyState === WebSocket.OPEN) connectedRows += 1;
      finalizeReadyBuckets(state, Date.now());
      const candidates = [
        ...provisionalRows(state),
        ...[...state.completedBuckets.values()],
      ...
      rows.push({
        provider,
        symbol,
        bucket_time: latest.bucket_time,
        bucket_end_time: latest.bucket_end_time,
  ...
  return {
    ...
    focus_pool_ready: focus.ready,
    focus_pool_round: focus.round,
    target_rows: [...PROVIDERS].reduce((sum, provider) => sum + (focus.byProvider[provider] || []).length, 0),
    active_rows: activeRows,
    connected_rows: connectedRows,
    flow_rows: rows.length,
    rows,
...
async function fetchBybitContractMeta(symbol) {
  ...
  const raw = await firstWorkingJson([
    ...
    `https://api.bytick.com/v5/market/tickers?category=${bybitCategory(symbol)}&symbol=${encodeURIComponent(native)}`,
  ], { timeoutMs: 5500 });
...
async function fetchGateContractMeta(symbol) {
  ...
  const raw = await firstWorkingJson([
    `https://api.gateio.ws/api/v4/futures/${gateSettle(symbol)}/contracts/${encodeURIComponent(contract)}`,
    `https://fx-api.gateio.ws/api/v4/futures/${gateSettle(symbol)}/contracts/${encodeURIComponent(contract)}`,
  ], { timeoutMs: 5500 });
  const item = Array.isArray(raw) ? raw[0] : firstDataObject(raw) || raw;
  if (!item || typeof item !== 'object') throw new Error('gate_contract_meta_empty');
  return normalizeContractMeta('gate', symbol, {
    ...
    next_funding_time: item.funding_next_apply,
    source_time: Date.now(),
    source: 'gate_futures_contract_detail',
  });
}
...
function pruneBinanceOfficialTakerRefreshDebt(focusSymbols) {
  const keep = new Set(Array.isArray(focusSymbols) ? focusSymbols : []);
  for (const symbol of [...binanceOfficialTakerRefreshDebt]) {
    if (!keep.has(symbol)) binanceOfficialTakerRefreshDebt.delete(symbol);
  }
  if (binanceOfficialTakerRefreshDebt.size === 0) {
    binanceOfficialTakerRefreshDebtSince = 0;
    binanceOfficialTakerLastDebtReason = '';
  }
}

function addBinanceOfficialTakerRefreshDebt(symbols, reason = '') {
  let added = 0;
  for (const raw of Array.isArray(symbols) ? symbols : []) {
    const symbol = symbolKey(raw);
    if (!symbol) continue;
    if (!binanceOfficialTakerRefreshDebt.has(symbol)) added += 1;
    binanceOfficialTakerRefreshDebt.add(symbol);
  }
  if (binanceOfficialTakerRefreshDebt.size > 0) {
    if (!binanceOfficialTakerRefreshDebtSince) binanceOfficialTakerRefreshDebtSince = Date.now();
    if (reason) binanceOfficialTakerLastDebtReason = String(reason).slice(0, 300);
  }
  return added;
}

function clearBinanceOfficialTakerRefreshDebt(symbol) {
  binanceOfficialTakerRefreshDebt.delete(symbolKey(symbol));
  if (binanceOfficialTakerRefreshDebt.size === 0) {
    binanceOfficialTakerRefreshDebtSince = 0;
    binanceOfficialTakerLastDebtReason = '';
  }
...
function binanceOfficialTakerHealthPayload() {
  ...
  return {
    ...
    pending_refresh_debt_symbols: focus.symbols.filter((symbol)=>binanceOfficialTakerRefreshDebt.has(symbol)),
    refresh_debt_since: binanceOfficialTakerRefreshDebtSince ? new Date(binanceOfficialTakerRefreshDebtSince).toISOString() : null,
    refresh_debt_last_reason: binanceOfficialTakerLastDebtReason || null,
    partial_cycles: binanceOfficialTakerPartialCycles,
    refresh_debt_recoveries: binanceOfficialTakerDebtRecoveries,
...
async function refreshBinanceOfficialTakerFocus(reason='scheduled', { missingOnly=false }={}) {
  ...
  const task=(async()=>{
    ...
    if(!focus.ready){
      ...
      binanceOfficialTakerStats.last_error=`binance_official_taker_focus_not_ready:${focus.symbols.length}/15`;
      return false;
    }

    pruneBinanceOfficialTakerRefreshDebt(focus.symbols);
    const debtBefore=focus.symbols.filter((symbol)=>binanceOfficialTakerRefreshDebt.has(symbol)).length;

    const scanSymbols=focus.symbols.filter((symbol)=>{
      const current=binanceOfficialTakerBySymbol.get(symbol);
      if(!missingOnly) return true;
      return binanceOfficialTakerRefreshDebt.has(symbol) || !binanceOfficialTakerEntryFresh(current);
    });
    ...
    for(let index=0;index<scanSymbols.length;index+=1){
      const symbol=scanSymbols[index];
      ...
      try{
        ...
        addBinanceOfficialTakerRefreshDebt([symbol], `${reason}:${symbol}:${message}`);

        const temporaryRelayStop =
          error?.backgroundDeferred ||
          error?.auxiliaryDeferred ||
          error?.internalBinanceRestGuard ||
          error?.internalBinanceRelayGuard ||
          [403,418,429,451].includes(Number(error?.status||0));

        if(error?.backgroundDeferred||error?.auxiliaryDeferred){
          binanceOfficialTakerStats.background_deferrals+=1;
        }

        if(temporaryRelayStop){
          // The old implementation broke here and forgot every unattempted
          // symbol. Keep the entire remainder as explicit refresh debt.
          const remainder=scanSymbols.slice(index+1);
          addBinanceOfficialTakerRefreshDebt(
            remainder,
            `${reason}:interrupted_after:${symbol}:${message}`,
          );
          interrupted=true;
          break;
        }
    ...
    if(!cycleComplete){
      binanceOfficialTakerPartialCycles+=1;
    }else if(debtBefore>0){
      binanceOfficialTakerDebtRecoveries+=1;
    }

    // A partial refresh is never signed off merely because the previous cache
    // is still within the 12-minute freshness window.
    if(health.ready && cycleComplete){
    ...
    if(health.ready && cycleComplete){
      binanceOfficialTakerStats.last_error='';
    }else{
      const parts=[...errors];
      if(pending.length){
        parts.push(`pending_refresh_debt:${pending.join(',')}`);
      }
      if(interrupted){
        parts.push('partial_cycle_interrupted_by_existing_relay_guard_or_capacity');
      }
      binanceOfficialTakerStats.last_error=parts.join(' | ').slice(0,700);
    }

    // Returning false while refresh debt exists schedules the existing bounded
    // recovery path even if old cache rows are still technically fresh.
    return health.ready && cycleComplete;
  })();
  ...
  try{
    ...
    if(binanceOfficialTakerInflight===task) binanceOfficialTakerInflight=null;
  }
}
...
function startBinanceOfficialTakerCollector() {
  ...
  const focusWatcher=setInterval(async()=>{
    const health=binanceOfficialTakerHealthPayload();
    if(!health.ready || Number(health.pending_refresh_debt_count||0)>0){
      const ready=await refreshBinanceOfficialTakerFocus('focus_missing_or_refresh_debt_watch',{missingOnly:true}).catch(()=>false);
      if(!ready) scheduleBinanceOfficialTakerRecovery();
    }
  },30_000);
  focusWatcher.unref?.();
...
const HOT_SCORE_FLOW_HYDRATE_MS = 5 * 60_000;
let hotScoreFlowHydrateAt = 0;
let hotScoreFlowHydrateInflight = null;
let hotScoreFlowHydrateError = '';
let hotScoreFlowHydrateRows = 0;

async function hydrateHotScoreFlowMetricsFromPersistence({ force = false } = {}) {
  if (!PERSISTENCE_ENABLED) return 0;
  const now = Date.now();
  if (!force && hotScoreFlowHydrateAt > 0 && now - hotScoreFlowHydrateAt < HOT_SCORE_FLOW_HYDRATE_MS) return hotScoreFlowHydrateRows;
  if (hotScoreFlowHydrateInflight) return await hotScoreFlowHydrateInflight;
  hotScoreFlowHydrateInflight = (async () => {
    const cutoff = new Date(Date.now() - 45 * 60_000).toISOString();
    const query = new URLSearchParams({
      select: 'provider,symbol,bucket_time,buy_quote,sell_quote,trade_count',
    ...
    const text = await response.text();
    if (!response.ok) throw new Error(`hot_score_flow_hydrate_http_${response.status}:${text.slice(0,180)}`);
    const rows = JSON.parse(text);
    const accepted = publishContractFlowHotScoreRows(Array.isArray(rows) ? rows : []);
    hotScoreFlowHydrateAt = Date.now();
    hotScoreFlowHydrateRows = accepted;
    hotScoreFlowHydrateError = '';
    return accepted;
  })().catch((error) => {
    hotScoreFlowHydrateError = String(error?.message || error).slice(0, 240);
    return 0;
  }).finally(() => { hotScoreFlowHydrateInflight = null; });
  return await hotScoreFlowHydrateInflight;
}
...
async function loadLatestPersistedFlowSnapshot(maxAgeMs) {
  ...
  const rows = JSON.parse(text);
  publishContractFlowHotScoreRows(Array.isArray(rows) ? rows : []);
...
const SHARED_CURRENT_SNAPSHOT_RPC = 'kaka_get_contract_current_metric_snapshot';
const SHARED_CURRENT_SNAPSHOT_CACHE_TTL_MS = 60_000;
const SHARED_CURRENT_SNAPSHOT_STALE_MS = 10 * 60_000;
const SHARED_CURRENT_SNAPSHOT_CACHE_MAX = 4;
const sharedCurrentSnapshotCache = new Map();
const sharedCurrentSnapshotInflight = new Map();

// Step1004.9 pressure pre-gate: /api/contract-flow/market-snapshot used to
// read the persisted 5m table once per user request. Keep the exact same
// full-universe response semantics, but refresh it only in the existing
// deep-market worker and let all users share the same in-memory snapshot.
const MARKET_FLOW_SHARED_REFRESH_MS = Math.max(60_000, Number(process.env.KAKA_MARKET_FLOW_SHARED_REFRESH_MS || 90_000));
const MARKET_FLOW_SHARED_STALE_MS = Math.max(10 * 60_000, MARKET_FLOW_SHARED_REFRESH_MS * 8);
const marketFlowSharedSnapshot = {
  started: false,
  timer: null,
  inflight: null,
  payload: null,
...
function sharedCurrentSnapshotCacheKey(maxAgeMinutes) {
  return `current:${maxAgeMinutes}`;
}

function pruneSharedCurrentSnapshotCache(now = Date.now()) {
  for (const [key, entry] of sharedCurrentSnapshotCache) {
    if (!entry || now - entry.cachedAt > SHARED_CURRENT_SNAPSHOT_STALE_MS) {
      sharedCurrentSnapshotCache.delete(key);
    }
  }
  while (sharedCurrentSnapshotCache.size > SHARED_CURRENT_SNAPSHOT_CACHE_MAX) {
    const oldestKey = [...sharedCurrentSnapshotCache.entries()]
      .sort((a, b) => (a[1]?.cachedAt || 0) - (b[1]?.cachedAt || 0))[0]?.[0];
    if (!oldestKey) break;
    sharedCurrentSnapshotCache.delete(oldestKey);
  }
}
...
function normalizeSharedCurrentMetricRow(raw) {
  ...
  const provider = providerKey(raw.provider);
  const symbol = symbolKey(raw.symbol);
  if (!provider || !symbol || !symbol.endsWith('USDT')) return null;
  const row = {
    ...raw,
    provider,
    symbol,
  ...
  for (const field of [
    ...
    'last_funding_rate_percent','funding_rate_percent',
  ]) {
    const value = asNumber(row[field]);
...
async function getSharedCurrentContractSnapshot({ maxAgeMinutes = 30 } = {}) {
  const safeMinutes = Math.max(5, Math.min(180, Number(maxAgeMinutes) || 30));
  const key = sharedCurrentSnapshotCacheKey(safeMinutes);
  const now = Date.now();
  pruneSharedCurrentSnapshotCache(now);
  const cached = sharedCurrentSnapshotCache.get(key);
  if (cached && now - cached.cachedAt < SHARED_CURRENT_SNAPSHOT_CACHE_TTL_MS) {
    return {
      ...cached.payload,
      cache_hit: true,
      cache_stale: false,
  ...
  if (sharedCurrentSnapshotInflight.has(key)) {
    return await sharedCurrentSnapshotInflight.get(key);
  }
  const promise = (async () => {
    try {
      const raw = await callSharedFlowRpc(
        SHARED_CURRENT_SNAPSHOT_RPC,
        { p_max_age_minutes: safeMinutes, p_limit: 5000 },
        30000,
      );
      if (!raw || typeof raw !== 'object' || raw.ok !== true) {
        throw new Error('shared_current_snapshot_invalid_payload');
      }
      const rows = (Array.isArray(raw.rows) ? raw.rows : [])
        .map(normalizeSharedCurrentMetricRow)
        .filter(Boolean);
      const payload = {
        ...raw,
        ok: true,
    ...
    business_policy_cross_quote_substitution: false,
      ...
      sharedCurrentSnapshotCache.set(key, { cachedAt: Date.now(), payload });
      pruneSharedCurrentSnapshotCache();
      return payload;
    } catch (error) {
      const stale = sharedCurrentSnapshotCache.get(key) || cached;
      if (stale && Date.now() - stale.cachedAt < SHARED_CURRENT_SNAPSHOT_STALE_MS) {
        return {
          ...stale.payload,
          cache_hit: true,
          cache_stale: true,
          cache_age_ms: Date.now() - stale.cachedAt,
          warning: 'shared_current_snapshot_refresh_failed_stale_cache_retained',
          refresh_error: String(error?.message || error).slice(0, 240),
        };
      }
      throw error;
    }
  })().finally(() => sharedCurrentSnapshotInflight.delete(key));
  sharedCurrentSnapshotInflight.set(key, promise);
...
async function buildFullUniverseFlowSnapshot({ maxAgeMs = 20 * 60_000 } = {}) {
  ...
  for (const state of states.values()) {
    ...
    if (!row) continue;
    const liveMeta = mergeContractMetaWithOpenInterest(
      getContractMetaFast(state.provider, state.symbol),
      state,
    );
    if (liveMeta) {
      for (const key of [
        ...
        'contract_oi_value',
        'open_interest_unit',
        'open_interest_value_unit',
        'open_interest_source_time',
        'last_funding_rate',
        'funding_rate',
...
async function refreshMarketFlowSharedSnapshot(reason = 'interval') {
  if (!PERSISTENCE_ENABLED) return null;
  if (marketFlowSharedSnapshot.inflight) return await marketFlowSharedSnapshot.inflight;
  marketFlowSharedSnapshot.refresh_attempts += 1;
  marketFlowSharedSnapshot.background_db_reads += 1;
  marketFlowSharedSnapshot.last_attempt_at = Date.now();
  marketFlowSharedSnapshot.inflight = (async () => {
    try {
      const snapshot = await buildFullUniverseFlowSnapshot({ maxAgeMs: 20 * 60_000 });
      const rowCount = Array.isArray(snapshot?.rows) ? snapshot.rows.length : 0;
      const coverage = snapshot?.coveredByProvider || {};
      const allProviders = [...PROVIDERS].every((provider) => Number(coverage[provider] || 0) > 0);
      if (rowCount <= 0 || !allProviders) throw new Error(`market_flow_shared_snapshot_incomplete:${rowCount}`);
      marketFlowSharedSnapshot.payload = {
        rows: snapshot.rows.map((row) => ({ ...row })),
        coveredByProvider: { ...coverage },
        row_count: rowCount,
        refresh_reason: reason,
      };
      marketFlowSharedSnapshot.cached_at = Date.now();
      marketFlowSharedSnapshot.last_success_at = Date.now();
      marketFlowSharedSnapshot.refresh_successes += 1;
      marketFlowSharedSnapshot.last_error = '';
      return marketFlowSharedSnapshot.payload;
    } catch (error) {
      marketFlowSharedSnapshot.refresh_failures += 1;
      marketFlowSharedSnapshot.last_error = String(error?.message || error).slice(0, 300);
      return marketFlowSharedSnapshot.payload;
    }
  })().finally(() => { marketFlowSharedSnapshot.inflight = null; });
  return await marketFlowSharedSnapshot.inflight;
}

function startMarketFlowSharedSnapshotMaintainer() {
  if (marketFlowSharedSnapshot.started) return;
  marketFlowSharedSnapshot.started = true;
  const first = setTimeout(() => refreshMarketFlowSharedSnapshot('startup').catch(() => {}), 5_000);
  first.unref?.();
  marketFlowSharedSnapshot.timer = setInterval(() => refreshMarketFlowSharedSnapshot('interval').catch(() => {}), MARKET_FLOW_SHARED_REFRESH_MS);
  marketFlowSharedSnapshot.timer.unref?.();
}

function marketFlowSharedSnapshotHealth() {
  const now = Date.now();
  const ageMs = marketFlowSharedSnapshot.cached_at > 0 ? Math.max(0, now - marketFlowSharedSnapshot.cached_at) : null;
  const payload = marketFlowSharedSnapshot.payload;
  const coverage = payload?.coveredByProvider || {};
  const providerCount = [...PROVIDERS].filter((provider) => Number(coverage[provider] || 0) > 0).length;
  return {
    ready: Boolean(payload) && providerCount === 5 && Number(payload?.row_count || 0) > 0 && ageMs != null && ageMs <= MARKET_FLOW_SHARED_STALE_MS,
    mode: 'background_supabase_refresh_shared_in_memory_user_read_only',
    refresh_every_seconds: Math.round(MARKET_FLOW_SHARED_REFRESH_MS / 1000),
    stale_seconds: Math.round(MARKET_FLOW_SHARED_STALE_MS / 1000),
    row_count: Number(payload?.row_count || 0),
    provider_count: providerCount,
    covered_by_provider: { ...coverage },
    cache_age_ms: ageMs,
    refresh_attempts: marketFlowSharedSnapshot.refresh_attempts,
    refresh_successes: marketFlowSharedSnapshot.refresh_successes,
    refresh_failures: marketFlowSharedSnapshot.refresh_failures,
    background_db_reads: marketFlowSharedSnapshot.background_db_reads,
    user_reads: marketFlowSharedSnapshot.user_reads,
    user_reads_trigger_database_requests: false,
    user_reads_trigger_exchange_requests: false,
    user_reads_trigger_exchange_connections: false,
    database_reads_scale_with_users: false,
    exchange_upstream_scales_with_users: false,
    last_attempt_at: marketFlowSharedSnapshot.last_attempt_at ? new Date(marketFlowSharedSnapshot.last_attempt_at).toISOString() : null,
    last_success_at: marketFlowSharedSnapshot.last_success_at ? new Date(marketFlowSharedSnapshot.last_success_at).toISOString() : null,
    last_error: marketFlowSharedSnapshot.last_error,
  };
}
...
export function getContractFlowHealth() {
  ...
  return {
    ...
    fixed_symbol_whitelist: false,
    market_snapshot_shared_cache: marketFlowSharedSnapshotHealth(),
    focus_pool_binding: focusFlowBindingPayload(),
    focus_pool_15_each_kept_active: true,
    focus_pool_extra_full_universe_rotation_preserved: true,
...
export async function handleContractFlow(req,res,url){
  ...
  if(url.pathname==='/api/contract-flow/current-snapshot'){
    ...
    try{
      ...
      sendJson(res,200,{ok:true,version:VERSION,...snapshot,backend_cycle:flowScanState.cycle,full_universe_scan:flowScanStatusPayload(),generated_at:snapshot.generated_at||new Date().toISOString()});
    }catch(error){
      sendJson(res,502,{ok:false,version:VERSION,error:'shared_current_contract_snapshot_unavailable',detail:String(error?.message||error).slice(0,500),exchange_requests_started:0});
    }
    return true;
  }
  ...
  if(url.pathname==='/api/contract-flow/market-snapshot'){
    ...
    // Repeated browser/App reads previously replaced the Binance batch before
    // its guarded 2.5-second connection queue could finish opening.
    startContractFlowUniverseScanner();
    ...
    marketFlowSharedSnapshot.user_reads += 1;
    const shared=marketFlowSharedSnapshot.payload;
    const sharedHealth=marketFlowSharedSnapshotHealth();
    if(!shared||sharedHealth.ready!==true){
      sendJson(res,503,{ok:false,version:VERSION,error:'shared_market_flow_snapshot_warming',quote_asset:'USDT',snapshot_triggers_rotation:false,snapshot_triggers_database_read:false,snapshot_triggers_exchange_request:false,shared_cache:sharedHealth,generated_at:new Date().toISOString()});return true;
    }
    const rows=providerFilter?shared.rows.filter((row)=>row.provider===providerFilter):shared.rows;
    sendJson(res,200,{
      ok:true,
      version:VERSION,
      quote_asset:'USDT',
      max_age_minutes:maxAgeMinutes,
      snapshot_triggers_rotation:false,
      snapshot_triggers_database_read:false,
      snapshot_triggers_exchange_request:false,
      user_reads_trigger_database_requests:false,
      user_reads_trigger_exchange_requests:false,
      database_reads_scale_with_users:false,
      rows,
      total_rows:rows.length,
      covered_by_provider:shared.coveredByProvider,
      full_universe_scan:flowScanStatusPayload(),
      shared_cache:sharedHealth,
      source:'five_platform_full_universe_rotating_public_trade_websocket_plus_background_shared_supabase_5m_cache',
      generated_at:new Date().toISOString(),
    });
    return true;
  }
  ...
  loadPersistedMetrics(state).catch(()=>{});
  let oiFirstPaintPromise=null;
  let ratioFirstPaintPromise=null;
  if(provider==='binance'){
    // Step650.8.15.36: OI is the row-level field required by unusual/funding/OI
    // lists, so it acquires the authenticated Edge critical lane first. Ratio is
    // still scheduled in the same request but cannot starve OI first paint.
    oiFirstPaintPromise=refreshBinanceOpenInterestCritical(state).catch(()=>null);
  ...
  if(waitMs>0){
    ...
    if(provider==='binance'&&!latestOpenInterestPatch(state)){
      waits.push(Promise.race([
        oiFirstPaintPromise||Promise.resolve(null),
        new Promise((resolve)=>setTimeout(resolve,Math.max(2500,waitMs))),
      ]));
    }
