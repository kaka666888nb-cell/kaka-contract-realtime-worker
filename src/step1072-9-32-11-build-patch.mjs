...
function replaceFunctionByBoundary(text, startNeedle, nextNeedle, replacement, label) {
  const startHits = count(text, startNeedle);
  const nextHits = count(text, nextNeedle);
  if (startHits !== 1 || nextHits !== 1) {
    throw new Error(`Step1072.9.32.11 RED function boundary mismatch ${label}: start=${startHits} next=${nextHits}`);
  }
  const start = text.indexOf(startNeedle);
  const end = text.indexOf(nextNeedle, start + startNeedle.length);
  if (start < 0 || end <= start) {
    throw new Error(`Step1072.9.32.11 RED invalid function boundary ${label}`);
  }
  const between = text.slice(start, end);
  if (count(between, 'Number(a.open_time ?? a.openTime ?? a.time ?? 0)') !== 1 ||
      count(between, 'Number(row.open_time ?? row.openTime ?? row.time ?? 0)') !== 1) {
    throw new Error(`Step1072.9.32.11 RED legacy timestamp semantics mismatch ${label}`);
  }
  const separator = between.includes('\r\n') ? '\r\n\r\n' : '\n\n';
  return text.slice(0, start) + replacement + separator + text.slice(end);
}
...
const rowsFunctionStart = 'function rowsWithTimes(raw) {';
const rowsFunctionNext = 'function takerCoverage(rows) {';
...
const red = {
  ...
  history_function_start: count(history, rowsFunctionStart),
  history_function_next: count(history, rowsFunctionNext),
  history_old_numeric_sort: count(history, 'Number(a.open_time ?? a.openTime ?? a.time ?? 0)'),
  history_old_numeric_row: count(history, 'Number(row.open_time ?? row.openTime ?? row.time ?? 0)'),
...
if (
  ...
  red.history_function_start !== 1 ||
  red.history_function_next !== 1 ||
  red.history_old_numeric_sort !== 1 ||
  red.history_old_numeric_row !== 1 ||
...
history = replaceFunctionByBoundary(history, rowsFunctionStart, rowsFunctionNext, newRowsWithTimes, 'rowsWithTimes');
...
const green = {
  ...
  old_numeric_row_remaining: count(history, 'Number(row.open_time ?? row.openTime ?? row.time ?? 0)'),
...
if (
  ...
  green.old_numeric_row_remaining !== 0 ||
