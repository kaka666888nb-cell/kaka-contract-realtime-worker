...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url?.startsWith('/ws-health')) {
    ...
    res.end(JSON.stringify({ ok: true, version: '650.8.15.62', coinbase_one_second_realtime_source: 'coinbase_exchange_ticker_per_match_plus_heartbeat', coinbase_all_directory_quotes_realtime_supported: true,
...
server.listen(PORT, () => {
  console.log(`Kaka market realtime worker 650.8.15.62 listening on ${PORT}`);
  setTimeout(() => {
