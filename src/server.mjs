...
const PROVIDERS = new Set(['binance', 'okx', 'gate', 'bitget']);
...
function okxInstId(symbol) { const [base, quote] = splitSymbol(symbol); return `${base}-${quote}-SWAP`; }
function gateContract(symbol) { const [base, quote] = splitSymbol(symbol); return `${base}_${quote}`; }
...
function normalizedMessage(provider, symbol, interval, a, closed = false, trades = 0) {
  const t = Number(a[0]);
  ...
  return JSON.stringify({
    stream: `${provider}:${symbol}:contract:${interval}`,
    provider,
    market: 'contract',
    data: {
...
function upstreamConfig(provider, symbol, interval) {
  if (provider === 'binance') return {
    url: `wss://fstream.binance.com/ws/${symbol.toLowerCase()}@kline_${interval}`,
    subscribe: null,
    parse(raw) {
      const message = JSON.parse(raw.toString());
      const k = message?.k || message?.data?.k;
      if (!k) return null;
      return normalizedMessage(provider, symbol, interval, [k.t,k.o,k.h,k.l,k.c,k.v,k.q], k.x, k.n);
    },
  };
  if (provider === 'okx') return {
    url: 'wss://ws.okx.com:8443/ws/v5/business',
    subscribe: { op: 'subscribe', args: [{ channel: okxChannel(interval), instId: okxInstId(symbol) }] },
    parse(raw) {
      const message = JSON.parse(raw.toString());
      const a = Array.isArray(message?.data) ? message.data[0] : null;
      if (!Array.isArray(a)) return null;
      return normalizedMessage(provider, symbol, interval, [a[0],a[1],a[2],a[3],a[4],a[5],a[7]], String(a[8]) === '1');
    },
  };
  if (provider === 'gate') return {
    url: 'wss://fx-ws.gateio.ws/v4/ws/usdt',
    subscribe: { time: Math.floor(Date.now()/1000), channel: 'futures.candlesticks', event: 'subscribe', payload: [gateInterval(interval), gateContract(symbol)] },
    parse(raw) {
      const message = JSON.parse(raw.toString());
      if (message?.channel !== 'futures.candlesticks' || message?.event !== 'update') return null;
      const r = Array.isArray(message.result) ? message.result[0] : message.result;
      if (!r) return null;
      const t = Number(r.t) * 1000;
      return normalizedMessage(provider, symbol, interval, [t,r.o,r.h,r.l,r.c,r.v,r.a ?? r.sum], false, r.n);
    },
  };
  return {
    url: 'wss://ws.bitget.com/v2/ws/public',
    subscribe: { op: 'subscribe', args: [{ instType: 'USDT-FUTURES', channel: bitgetChannel(interval), instId: symbol }] },
    parse(raw) {
      const message = JSON.parse(raw.toString());
      const a = Array.isArray(message?.data) ? message.data[0] : null;
      if (!Array.isArray(a)) return null;
      return normalizedMessage(provider, symbol, interval, [a[0],a[1],a[2],a[3],a[4],a[5],a[6]], false);
    },
  };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url?.startsWith('/health')) {
    ...
    res.end(JSON.stringify({ ok:true, protocol:'kaka.market.realtime.v1', providers:[...PROVIDERS], time:new Date().toISOString() }));
    return;
  }
  if (req.url?.startsWith('/diagnose') || req.url?.startsWith('/browser-test')) {
    res.writeHead(200, {'content-type':'text/html; charset=utf-8'});
    res.end('<!doctype html><meta charset="utf-8"><title>Kaka contract realtime</title><h1>Kaka contract realtime worker</h1><p>Providers: Binance / OKX / Gate / Bitget</p><p>Use /ws?provider=okx&market=contract&symbol=BTCUSDT&interval=15m</p>');
    return;
  }
...
wss.on('connection', (client, _req, parsedUrl) => {
  ...
  const market = parsedUrl.searchParams.get('market') || 'contract';
  if (!provider || !symbol || !VALID_INTERVALS.has(interval) || market !== 'contract') {
    client.close(1008, 'invalid contract channel'); return;
  }
  const cfg = upstreamConfig(provider, symbol, interval);
  let upstream;
  let heartbeat;
  try {
  ...
  upstream.on('open', () => {
    ...
    client.send(JSON.stringify({ type:'ready', provider, market:'contract', symbol, interval, protocol:'kaka.market.realtime.v1' }));
    heartbeat = setInterval(() => {
      if (upstream.readyState === WebSocket.OPEN) {
        if (provider === 'okx') upstream.send('ping');
        else if (provider === 'bitget') upstream.send('ping');
        else upstream.ping();
      }
...
server.listen(PORT, () => console.log(`Kaka contract realtime worker 512.2 listening on ${PORT}`));
