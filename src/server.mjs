...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url?.startsWith('/ws-health')) {
    ...
    res.end(JSON.stringify({ ok: true, version: '650.8.15.2', binance_shared_ws: binanceSharedWsHealth(), time: new Date().toISOString() }));
    return;
  }
...
wss.on('connection', async (client, req, parsedUrl) => {
  ...
  upstream.on('open', () => {
    ...
    if (cfg.tradeMode === true) {
      ...
      // Step650.8.15.2: the WS-only child must never become a second Binance REST
...
server.listen(PORT, () => console.log(`Kaka market realtime worker 650.8.15.2 listening on ${PORT}`));
