...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url?.startsWith('/ws-health')) {
    ...
    res.end(JSON.stringify({ ok: true, version: '650.8.15.55', binance_shared_ws: binanceSharedWsHealth(), bybit_second_history: getBybitSecondHistoryHealth(), provider_request_governor: getProviderGovernorHealth(), time: new Date().toISOString() }));
    return;
  }
...
server.listen(PORT, () => {
  console.log(`Kaka market realtime worker 650.8.15.55 listening on ${PORT}`);
  setTimeout(() => {
