...
const RANKED_ROUTE = '/api/stock-catalog-v2/ranked-page';
...
// Step1073 R40: Coinbase market failures must not hammer a repeatedly-500ing
// provider every 15 minutes. The shared last-good snapshot remains readable.
const COINBASE_MARKET_5XX_BACKOFF_BASE_MS = Math.max(
  COINBASE_MARKET_REFRESH_MS,
  Math.min(6 * 60 * 60_000, Number(process.env.KAKA_STOCK_MARKET_5XX_BACKOFF_BASE_MS || 30 * 60_000) || 30 * 60_000),
);
const COINBASE_MARKET_5XX_BACKOFF_MAX_MS = Math.max(
  COINBASE_MARKET_5XX_BACKOFF_BASE_MS,
  Math.min(12 * 60 * 60_000, Number(process.env.KAKA_STOCK_MARKET_5XX_BACKOFF_MAX_MS || 6 * 60 * 60_000) || 6 * 60 * 60_000),
);
...
// Step1042.1.6: failed/interrupted stock catalog refreshes can leave orphaned stage rows.
// User reads never run this cleanup. The background stock collector deletes only rows older
...
const COMMIT_MODE = 'validated_atomic_sha256_diff_replace_changed_only';
const STAGE_PERSISTENCE_MODE = 'unlogged_ephemeral';
const HASH_SIDECAR_TABLE = 'kaka_exchange_stock_catalog_v2_hashes';
...
let coinbaseMarketConsecutive5xxFailures = 0;
let coinbaseMarketNextAllowedAt = 0;
let coinbaseMarketBackoffSkips = 0;
let runtimeHealthPersistSuccesses = 0;
let runtimeHealthPersistFailures = 0;
let lastRuntimeHealthPersistError = '';
...
let stageRetentionTimer = null;
let stageRetentionInflight = null;
let stageRetentionRuns = 0;
let stageRetentionRowsDeleted = 0;
let stageRetentionFailures = 0;
let stageRetentionLastStartedAt = '';
let stageRetentionLastSucceededAt = '';
let stageRetentionLastError = '';
let stageRetentionLastDeletedRows = 0;
const coinbaseExactMetadataLoaded = new Set();
const coinbaseExactMetadataInflight = new Map();
// Step1035.19.5: /api/asset-klines lives in the isolated exchange-assets child process,
// while the full stock-catalog collector lives in the parent process. Module memory is not
// shared across those processes. Keep a tiny Supabase-backed read replica for only the fixed
// Coinbase core-equity Kline pool inside the isolated process. This replica never calls
// Coinbase, is singleflight, and is refreshed on a long TTL so user reads do not scale DB or
// exchange upstream work.
const COINBASE_CORE_KLINE_REPLICA_TTL_MS = Math.max(30 * 60_000, Number(process.env.KAKA_COINBASE_CORE_KLINE_REPLICA_TTL_MS || 6 * 60 * 60_000));
const COINBASE_CORE_KLINE_REPLICA_MIN_ROWS = 20;
let coinbaseCoreKlineReplicaInflight = null;
let coinbaseCoreKlineReplicaLoadedAtMs = 0;
let coinbaseCoreKlineReplicaRows = 0;
let coinbaseCoreKlineReplicaLoads = 0;
let coinbaseCoreKlineReplicaFailures = 0;
let coinbaseCoreKlineReplicaLastError = '';
...
// Step1041.6: rank only the already-shared cash-stock catalog. No user read
// starts Gate/Coinbase work. The frozen order contains only exact identities;
// each page materializes at most 50 current rows from shared memory.
const STOCK_RANK_PAGE_MAX = 50;
const STOCK_RANK_TTL_MS = Math.max(60_000, Number(process.env.KAKA_STOCK_RANK_TTL_MS || 5 * 60_000));
const STOCK_RANK_MAX_SNAPSHOTS = Math.max(2, Math.min(12, Number(process.env.KAKA_STOCK_RANK_MAX_SNAPSHOTS || 8)));
const stockRankSnapshots = new Map();
const stockRankCurrentByScope = new Map();
let stockRankSeq = 0;
let stockRankReads = 0;
let stockRankBuilds = 0;
let stockRankHits = 0;
let stockRankExpired = 0;
...
async function cleanupExpiredStageBatch(provider) {
  if (!SUPABASE_CONFIGURED) throw new Error('supabase_not_configured');
  const cutoff = new Date(Date.now() - STAGE_RETENTION_MS).toISOString();
  const response = await supabaseFetch(`rpc/${STAGE_RETENTION_RPC}`, {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ p_provider: provider, p_cutoff: cutoff, p_limit: STAGE_RETENTION_BATCH }),
  });
  const raw = await response.text();
  if (!response.ok) throw new Error(`stock_stage_retention_${provider}_${response.status}:${safeText(raw, 180)}`);
  const parsed = Number(JSON.parse(raw));
  return Number.isFinite(parsed) && parsed >= 0 ? Math.trunc(parsed) : 0;
}
async function runStageRetention(reason = 'interval') {
  if (stageRetentionInflight) return stageRetentionInflight;
  stageRetentionInflight = (async () => {
    stageRetentionRuns += 1;
    stageRetentionLastStartedAt = isoNow();
    stageRetentionLastDeletedRows = 0;
    stageRetentionLastError = '';
    try {
      let deleted = 0;
      for (const provider of ['gate','coinbase']) {
        for (let i = 0; i < STAGE_RETENTION_MAX_BATCHES_PER_RUN; i += 1) {
          const count = await cleanupExpiredStageBatch(provider);
          deleted += count;
          if (count < STAGE_RETENTION_BATCH) break;
        }
      }
      stageRetentionRowsDeleted += deleted;
      stageRetentionLastDeletedRows = deleted;
      stageRetentionLastSucceededAt = isoNow();
      stageRetentionLastError = '';
      return deleted;
    } catch (error) {
      stageRetentionFailures += 1;
      stageRetentionLastError = `${reason}:${safeText(error?.message || error, 220)}`;
      return 0;
    } finally {
      stageRetentionInflight = null;
    }
  })();
  return stageRetentionInflight;
}
...
async function refreshCoinbaseMarketMap(reason = 'market_interval') {
  ...
  if (coinbaseMarketNextAllowedAt > nowMs) {
    coinbaseMarketBackoffSkips += 1;
    return {
      ok: true,
      skipped: true,
      reason: 'coinbase_market_5xx_backoff',
      next_retry_at: new Date(coinbaseMarketNextAllowedAt).toISOString(),
  ...
  coinbaseMarketInflight = (async () => {
    ...
    try {
      ...
      coinbaseMarketRefreshFailures += 1;
      lastCoinbaseMarketError = safeText(error?.message || error, 260);
      const is5xx = coinbaseMarketErrorIs5xx(error);
      if (is5xx) {
        coinbaseMarketConsecutive5xxFailures += 1;
        coinbaseMarketNextAllowedAt = Date.now() + coinbaseMarketBackoffDelayMs();
      } else {
        coinbaseMarketConsecutive5xxFailures = 0;
        coinbaseMarketNextAllowedAt = 0;
      }
      await persistRuntimeHealth({
        persistence_mode: 'failure_runtime_health_only',
        last_reason: reason,
        error_class: is5xx ? 'coinbase_http_5xx' : 'other',
      });
      throw error;
...
async function restoreState() {
  ...
  try {
    const response = await supabaseFetch(`${STATE_TABLE}?singleton=eq.default&select=payload,coinbase_market_snapshot_gzip_b64,coinbase_market_snapshot_rows,coinbase_market_snapshot_fetched_at,runtime_health,runtime_health_updated_at&limit=1`);
    if (!response.ok) return;
    const rows = await response.json(); const stateRow = rows?.[0]; if (!stateRow) return; const row = stateRow.payload && typeof stateRow.payload === 'object' ? stateRow.payload : {}; const runtimeCoinbase = stateRow.runtime_health?.coinbase_market && typeof stateRow.runtime_health.coinbase_market === 'object' ? stateRow.runtime_health.coinbase_market : {};
...
export async function resolveCoinbaseEquityCandleRoute(nativeSymbol, securityTicker, assetId) {
  ...
  let current = sharedRowByNative.get(`coinbase|${native}`) || null;
  // In the isolated exchange-assets process the parent catalog map is a different address
  // space. Warm the fixed core read replica before declaring the exact row missing.
  if (!current && COINBASE_CORE_EQUITY_TICKERS.has(ticker)) {
    try { await ensureCoinbaseCoreKlineReplica(); } catch { /* reason below stays explicit */ }
    current = sharedRowByNative.get(`coinbase|${native}`) || null;
  }
  const baseRoute = coinbaseEquityCandleRouteFromRow(current, native, ticker, exactAssetId);
  if (baseRoute.identity_verified !== true) return baseRoute;

  // Detailed metadata is optional. The core replica already includes provider_metadata for
  // only 34 rows, but a parent-process exact metadata hit may still enrich it without ever
  // becoming a hard dependency.
  let detailed = current;
  try {
    const hydrated = await loadCoinbaseExactMetadata(native);
    if (hydrated && compact(hydrated?.asset_id) === exactAssetId && compact(hydrated?.exchange_symbol) === native && normalizeTicker(hydrated?.display_symbol || hydrated?.base_asset) === ticker) detailed = hydrated;
  } catch { /* exact replicated identity remains authoritative */ }

  const detailedRoute = coinbaseEquityCandleRouteFromRow(detailed, native, ticker, exactAssetId);
  const routeProductIds = detailedRoute.identity_verified === true && detailedRoute.route_product_ids.length
    ? detailedRoute.route_product_ids
    : baseRoute.route_product_ids;
  const quote = detailedRoute.identity_verified === true ? detailedRoute.quote : baseRoute.quote;
  const alias = detailedRoute.identity_verified === true ? detailedRoute.alias : baseRoute.alias;
  return {
    ...
    route_product_ids: routeProductIds,
    canonical_product_id: native,
    identity_source: compact(process.env.KAKA_ISOLATED_COLLECTOR_ROLE) === 'exchange-assets'
      ? 'isolated_exchange_assets_core_supabase_read_replica_exact_identity'
      : 'parent_shared_coinbase_catalog_exact_identity',
  };
}


function stockRankSortKey(raw) {
  ...
  if (['volume_desc','turnover_desc','volume','turnover'].includes(value)) return 'volume_desc';
  if (['name_asc','symbol_asc','default','name','symbol'].includes(value)) return 'name_asc';
  return 'name_asc';
}

function stockRankNullable(a, b, descending = false) {
  const av = finite(a); const bv = finite(b);
...
function stockRankIdentity(row) {
  return `${lower(row?.provider)}|${compact(row?.exchange_symbol)}`;
}

function stockRankMetric(row, sort) {
  if (sort === 'change_desc' || sort === 'change_asc') return finite(row?.reference_change_24h_ratio);
  if (sort === 'volume_desc') return nonneg(row?.reference_turnover_24h);
  return null;
}

function stockRankRows({ provider = 'all', sort = 'name_asc' } = {}) {
  const p = lower(provider) || 'all';
  const sortKey = stockRankSortKey(sort);
  const rows = [];
  for (const row of sharedRowByNative.values()) {
    if (lower(row?.asset_class) !== 'equity_cash') continue;
    if (p !== 'all' && lower(row?.provider) !== p) continue;
    if (!compact(row?.exchange_symbol)) continue;
    rows.push(row);
  }
  rows.sort((a, b) => {
    let cmp = 0;
    if (sortKey === 'change_desc') cmp = stockRankNullable(a?.reference_change_24h_ratio, b?.reference_change_24h_ratio, true);
    else if (sortKey === 'change_asc') cmp = stockRankNullable(a?.reference_change_24h_ratio, b?.reference_change_24h_ratio, false);
    else if (sortKey === 'volume_desc') cmp = stockRankNullable(a?.reference_turnover_24h, b?.reference_turnover_24h, true);
    if (!cmp && sortKey !== 'name_asc') cmp = stockRankNullable(a?.reference_turnover_24h, b?.reference_turnover_24h, true);
    if (!cmp) {
      const at = compact(a?.display_symbol || a?.base_asset || a?.exchange_symbol).toUpperCase();
      const bt = compact(b?.display_symbol || b?.base_asset || b?.exchange_symbol).toUpperCase();
      cmp = at.localeCompare(bt);
    }
    if (!cmp) cmp = stockRankIdentity(a).localeCompare(stockRankIdentity(b));
    return cmp;
  });
  return { sortKey, rows };
}

function pruneStockRankSnapshots() {
  const now = Date.now();
  for (const [key, value] of stockRankSnapshots.entries()) {
    if (now - Number(value?.created_at_ms || 0) > STOCK_RANK_TTL_MS) {
      stockRankSnapshots.delete(key);
      for (const [scope, version] of stockRankCurrentByScope.entries()) if (version === key) stockRankCurrentByScope.delete(scope);
      stockRankExpired += 1;
    }
  }
  if (stockRankSnapshots.size <= STOCK_RANK_MAX_SNAPSHOTS) return;
  const oldest = [...stockRankSnapshots.entries()].sort((a,b)=>Number(a[1]?.created_at_ms||0)-Number(b[1]?.created_at_ms||0));
  while (oldest.length > STOCK_RANK_MAX_SNAPSHOTS) {
    const [key] = oldest.shift(); stockRankSnapshots.delete(key);
    for (const [scope, version] of stockRankCurrentByScope.entries()) if (version === key) stockRankCurrentByScope.delete(scope);
  }
}

function stockRankSourceVersion() {
  return `${lastRefreshId || 'catalog'}|${lastCoinbaseMarketSucceededAt || ''}|${lastGateSessionSucceededAt || ''}`;
}

function stockRankScopeKey({ provider = 'all', sort = 'name_asc' } = {}) {
  return `${lower(provider) || 'all'}|${stockRankSortKey(sort)}`;
}

function createStockRankSnapshot({ provider = 'all', sort = 'name_asc' } = {}) {
  pruneStockRankSnapshots();
  const { sortKey, rows } = stockRankRows({ provider, sort });
  stockRankSeq += 1;
  const created = Date.now();
  const rankVersion = `sr-${lastRefreshId || 'catalog'}-${Date.parse(lastCoinbaseMarketSucceededAt || '') || 0}-${stockRankSeq}`;
  const snapshot = {
    rank_version: rankVersion,
    provider: lower(provider) || 'all',
    sort: sortKey,
    created_at_ms: created,
    source_version: stockRankSourceVersion(),
    order: rows.map((row) => ({
      rank_identity: stockRankIdentity(row),
      rank_metric_value: stockRankMetric(row, sortKey),
    })),
  };
  stockRankSnapshots.set(rankVersion, snapshot);
  stockRankCurrentByScope.set(stockRankScopeKey({ provider, sort: sortKey }), rankVersion);
  stockRankBuilds += 1;
  pruneStockRankSnapshots();
  return snapshot;
}

function getOrCreateCurrentStockRankSnapshot({ provider = 'all', sort = 'name_asc' } = {}) {
  pruneStockRankSnapshots();
  const scope = stockRankScopeKey({ provider, sort });
  const version = stockRankCurrentByScope.get(scope);
  const snapshot = version ? stockRankSnapshots.get(version) : null;
  if (snapshot && snapshot.source_version === stockRankSourceVersion() && Date.now() - Number(snapshot.created_at_ms || 0) <= STOCK_RANK_TTL_MS) {
    stockRankHits += 1;
    return snapshot;
  }
  return createStockRankSnapshot({ provider, sort });
}

function getStockRankSnapshot(rankVersion, { provider = 'all', sort = 'name_asc' } = {}) {
  if (!rankVersion) return null;
  pruneStockRankSnapshots();
  const snapshot = stockRankSnapshots.get(rankVersion);
  if (!snapshot) return null;
  if (snapshot.provider !== (lower(provider) || 'all') || snapshot.sort !== stockRankSortKey(sort)) return null;
  stockRankHits += 1;
  return snapshot;
}

function stockRankedPayload(url) {
  const provider = lower(url.searchParams.get('provider')) || 'all';
  if (!['all','coinbase','gate'].includes(provider)) return { status:400, payload:{ ok:false,version:VERSION,error:'unsupported_stock_rank_provider',provider } };
  const sort = stockRankSortKey(url.searchParams.get('sort'));
  const offset = Math.max(0, Math.trunc(Number(url.searchParams.get('offset') || 0) || 0));
  const limit = Math.max(1, Math.min(STOCK_RANK_PAGE_MAX, Math.trunc(Number(url.searchParams.get('limit') || 50) || 50)));
  const requested = compact(url.searchParams.get('rank_version'));
  if (offset > 0 && !requested) return { status:400, payload:{ ok:false,version:VERSION,error:'rank_version_required_after_first_page',restart_from_offset:0,user_read_upstream_requests:0 } };
  let snapshot = getStockRankSnapshot(requested, { provider, sort });
  if (requested && !snapshot) return { status:409, payload:{ ok:false,version:VERSION,error:'rank_version_expired_or_scope_mismatch',rank_version:requested,restart_from_offset:0,user_read_upstream_requests:0 } };
  if (!snapshot) snapshot = getOrCreateCurrentStockRankSnapshot({ provider, sort });
  const slice = snapshot.order.slice(offset, offset + limit);
  const items = slice.map((entry, index) => {
    const row = sharedRowByNative.get(entry.rank_identity) || null;
    return {
  ...
  return { status:200, payload:{
    ok:true,version:VERSION,data_version:DATA_VERSION,schema_version:'step1041_6_shared_cash_stock_rank_page_v1',
    provider,asset_class:'equity_cash',sort,rank_version:snapshot.rank_version,
    rank_snapshot_created_at:new Date(snapshot.created_at_ms).toISOString(),rank_snapshot_ttl_ms:STOCK_RANK_TTL_MS,
    total_items:snapshot.order.length,offset,limit,returned_items:items.length,
    has_more:offset+items.length<snapshot.order.length,next_offset:offset+items.length<snapshot.order.length?offset+items.length:null,
    supported_sorts:['name_asc','change_desc','change_asc','volume_desc'],market_cap_sort_supported:false,
    volume_metric:'reference_turnover_24h_only_no_cross_currency_share_volume_fallback',
...
export async function handleStockCatalogV2(req, res, url) {
  if (![HEALTH_ROUTE,CURRENT_ROUTE,TICKERS_ROUTE,EXACT_ROUTE,SELF_TEST_ROUTE,RANKED_ROUTE].includes(url.pathname)) return false;
  ...
  if (url.pathname === RANKED_ROUTE) {
    stockRankReads += 1;
    const result = stockRankedPayload(url);
...
export function startStockCatalogV2Collector() {
  ...
  restoreState().finally(async () => {
    ...
    refreshTimer.unref?.();
    const stageRetentionStart = setTimeout(() => runStageRetention('startup').catch(() => {}), 35_000);
    stageRetentionStart.unref?.();
    stageRetentionTimer = setInterval(() => runStageRetention('interval').catch(() => {}), STAGE_RETENTION_INTERVAL_MS);
    stageRetentionTimer.unref?.();
...
export function runStockCatalogV2SelfTest() {
  ...
  const tests = [
    ...
    ['coinbase_compact_market_snapshot_persisted_in_dedicated_column', String(persistState).includes('persistCoinbaseSnapshot')],
    ['coinbase_general_state_payload_excludes_snapshot_blob', String(persistState).includes('delete payload.coinbase_market_snapshot_gzip_b64')],
    ['coinbase_market_failure_uses_tiny_runtime_health_sidecar', String(refreshCoinbaseMarketMap).includes('persistRuntimeHealth') && !String(refreshCoinbaseMarketMap).includes("await persistState({ last_coinbase_market_reason: reason }); throw error")],
    ['coinbase_market_5xx_exponential_backoff', COINBASE_MARKET_5XX_BACKOFF_BASE_MS >= COINBASE_MARKET_REFRESH_MS && COINBASE_MARKET_5XX_BACKOFF_MAX_MS >= COINBASE_MARKET_5XX_BACKOFF_BASE_MS],
    ...
    ['stock_stage_persistence_contract_unlogged_ephemeral', STAGE_PERSISTENCE_MODE === 'unlogged_ephemeral'],
    ['stock_commit_contract_sha256_diff_only', COMMIT_MODE === 'validated_atomic_sha256_diff_replace_changed_only'],
    ['stock_hash_sidecar_contract_declared', HASH_SIDECAR_TABLE === 'kaka_exchange_stock_catalog_v2_hashes'],
    ...
    ['coinbase_exchange_assets_isolation_core_replica_available', typeof ensureCoinbaseCoreKlineReplica === 'function' && typeof getCoinbaseCoreKlineReplicaHealth === 'function'],
    ['coinbase_exchange_assets_replica_is_supabase_only', String(ensureCoinbaseCoreKlineReplica).includes('supabaseFetch') && !String(ensureCoinbaseCoreKlineReplica).includes('timedFetchJson') && !String(ensureCoinbaseCoreKlineReplica).includes('COINBASE_HOST')],
    ['coinbase_isolated_resolver_warms_replica_before_missing', String(resolveCoinbaseEquityCandleRoute).includes('ensureCoinbaseCoreKlineReplica')],
