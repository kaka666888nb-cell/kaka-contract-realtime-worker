...
// Step650.8.13：历史归档与实时WebSocket保持独立；REST桥接只保留一个官方精确交易对端点，
...
async function fetchBuffer(url, timeoutMs = FETCH_TIMEOUT_MS) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { accept: 'application/zip,application/octet-stream,*/*', 'user-agent': 'KakaWeb3-Kline-Seed/650.8.13' },
    });
...
export async function getBinanceContractKlineSeed({ symbol, interval = '15m', end = Date.now(), limit = 500, forceRestValidation = false, signal = null, maxRestCalls = 1 } = {}) {
  ...
  const task = (async () => {
    ...
    // Step650.8.13：冷启动仍优先官方当前窗口，但必须验证它真的覆盖请求起点且内部连续。
    // 仅返回当前一根属于 partial，不能阻止归档与后续精确 symbol 候选继续补齐。
    if (nearNow && normalizedInterval !== '1s' && !persisted.length) {
      // Step650.8.13: the production validation proved the exact 15m/240 relay path.
    ...
    const finalCoverage = nearNow
      ...
      : null;
    // Step650.8.13：临近当前的快照只有在最近窗口连续时才持久化。
