...
async function binanceRestJsonFetch(url, timeout = 15_000, source = 'legacy_market_rest') {
  // Step650.8.12: Binance Spot and Contract public HTTP both use the same
...
function aggregateTradesToSecondRows(trades, provider, market, symbol, end, limit) {
  ...
  // Step650.8.12: only seconds with real official trades become candles.
...
export async function fetchMarketKlines(provider, market, symbol, interval, end, limit, options = {}) {
  ...
  if (provider === 'binance' && market === 'contract') {
    // Step650.8.12：Binance 合约历史K线先读官方日/月归档；若持久快照尾部已有实时蜡烛但内部仍断层，则从第一个缺口开始补官方当前日HTTP桥接，再启动按需实时K线WebSocket。
    ...
    // Step650.8.12: never fall through to the generic native Binance REST path.
...
export async function handleMarketApi(req, res, url) {
  // Step650.8.12: this generic market handler must claim only routes it owns.
