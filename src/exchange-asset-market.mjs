// Planned Step1026: exact-identity shared market productization for non-crypto assets.
...
function parseCoinbaseProduct(p,productId){if(!p||typeof p!=='object')return null;const id=upper(p.product_id);if(!id||id!==productId)return null;const equity=p.equity_product_details&&typeof p.equity_product_details==='object'?p.equity_product_details:null;const future=p.future_product_details&&typeof p.future_product_details==='object'?p.future_product_details:null;const fcm=p.fcm_trading_session_details&&typeof p.fcm_trading_session_details==='object'?p.fcm_trading_session_details:null;return{source:'coinbase_advanced_trade_product',product_id:id,product_type:safeText(p.product_type),product_venue:safeText(p.product_venue),status:safeText(p.status),base_currency_id:safeText(p.base_currency_id),quote_currency_id:safeText(p.quote_currency_id),price:pos(p.price),price_change_24h_ratio:num(p.price_percentage_change_24h)!=null?num(p.price_percentage_change_24h)/100:null,volume_24h:nonneg(p.volume_24h),turnover_24h:nonneg(p.approximate_quote_24h_volume),base_increment:pos(p.base_increment),quote_increment:pos(p.quote_increment),price_increment:pos(p.price_increment),base_min_size:pos(p.base_min_size),base_max_size:pos(p.base_max_size),is_disabled:bool(p.is_disabled),view_only:bool(p.view_only),cancel_only:bool(p.cancel_only),limit_only:bool(p.limit_only),post_only:bool(p.post_only),trading_disabled:bool(p.trading_disabled),auction_mode:bool(p.auction_mode),equity_product_details:equity?{is_24_hour:bool(equity.is_24_hour),is_early_close:bool(equity.is_early_close),early_close_time:safeText(equity.early_close_time),current_trading_session:safeText(equity.current_trading_session),is_halted:bool(equity.is_halted),is_liquidate_only:bool(equity.is_liquidate_only),security:safeText(equity.security)}:null,future_product_details:future?{contract_expiry:safeText(future.contract_expiry),contract_size:num(future.contract_size),contract_root_unit:safeText(future.contract_root_unit),group_description:safeText(future.group_description)}:null,fcm_trading_session_details:fcm?{is_session_open:bool(fcm.is_session_open),open_time:safeText(fcm.open_time),close_time:safeText(fcm.close_time)}:null};}
function parseCoinbaseBook(payload,productId,limit=20){const pb=payload?.pricebook??payload;const id=upper(pb?.product_id??payload?.product_id);if(id&&id!==productId)return null;return{source:'coinbase_advanced_trade_public_product_book',product_id:id||productId,timestamp_ms:Date.parse(str(pb?.time))||null,bids:levels(pb?.bids,'bid',limit),asks:levels(pb?.asks,'ask',limit),mid_market:pos(payload?.mid_market),spread_bps:nonneg(payload?.spread_bps),spread_absolute:nonneg(payload?.spread_absolute)};}
...
async function buildCoinbase(identity){const productId=identity.nativeSymbol;const publicProductPath=`/api/v3/brokerage/market/products/${encodeURIComponent(productId)}`;const publicBookPath='/api/v3/brokerage/market/product_book';const publicTradesPath=`/api/v3/brokerage/market/products/${encodeURIComponent(productId)}/ticker`;const tasks=[coinbaseFetch(publicProductPath),coinbaseFetch(publicBookPath,`?product_id=${encodeURIComponent(productId)}&limit=20`),coinbaseFetch(publicTradesPath,'?limit=20')];if(COINBASE_CDP_CONFIGURED)tasks.push(coinbaseFetch(`/api/v3/brokerage/products/${encodeURIComponent(productId)}`));const rs=await Promise.allSettled(tasks);let rules=rs[0].status==='fulfilled'?parseCoinbaseProduct(rs[0].value,productId):null;if(rs[3]?.status==='fulfilled'){const privateFacts=parseCoinbaseProduct(rs[3].value,productId);if(privateFacts)rules={...rules,...privateFacts,source:'coinbase_advanced_trade_authenticated_product_official_fields'};}const book=rs[1].status==='fulfilled'?parseCoinbaseBook(rs[1].value,productId):null;const trades=rs[2].status==='fulfilled'?parseCoinbaseTrades(rs[2].value,productId):[];const ticker=rules?{source:rules.source,last_price:rules.price,price_change_24h_ratio:rules.price_change_24h_ratio,high_24h:null,low_24h:null,open_24h:null,volume_24h:rules.volume_24h,turnover_24h:rules.turnover_24h,best_bid:book?.bids?.[0]?.price??null,best_ask:book?.asks?.[0]?.price??null,best_bid_size:book?.bids?.[0]?.quantity??null,best_ask_size:book?.asks?.[0]?.quantity??null,timestamp_ms:book?.timestamp_ms??null,mark_price:null,index_price:null,open_interest:null,open_interest_value:null,funding_rate:null,next_funding_time_ms:null}:null;const hours=rules?{source:rules.source,equity:rules.equity_product_details,fcm:rules.fcm_trading_session_details}:null;return{ticker,orderbook:book,trades,rules,status:rules?{source:rules.source,status:rules.status,is_disabled:rules.is_disabled,view_only:rules.view_only,cancel_only:rules.cancel_only,limit_only:rules.limit_only,post_only:rules.post_only,trading_disabled:rules.trading_disabled,auction_mode:rules.auction_mode,equity_product_details:rules.equity_product_details}:null,trading_hours:hours,derivatives:rules?.future_product_details?{source:rules.source,future_product_details:rules.future_product_details}:null,capabilities:{ticker:capability(ticker?'official_shared':'temporarily_unavailable',publicProductPath),orderbook:capability(book?'official_shared':'temporarily_unavailable',publicBookPath),trades:capability(trades.length?'official_shared':'temporarily_unavailable',publicTradesPath),rules:capability(rules?'official_shared':'temporarily_unavailable',COINBASE_CDP_CONFIGURED?'/api/v3/brokerage/products/{product_id}':publicProductPath),status:capability(rules?'official_shared':'temporarily_unavailable',COINBASE_CDP_CONFIGURED?'/api/v3/brokerage/products/{product_id}':publicProductPath),trading_hours:capability(hours?'official_shared':'temporarily_unavailable',COINBASE_CDP_CONFIGURED?'/api/v3/brokerage/products/{product_id}':publicProductPath),derivatives:rules?.future_product_details?capability('official_shared',COINBASE_CDP_CONFIGURED?'/api/v3/brokerage/products/{product_id}':publicProductPath):capability('not_applicable')}};}
...
async function fetchBatchCoinbase(identity,symbols){const path='/api/v3/brokerage/market/products';const q=`?product_ids=${encodeURIComponent(symbols.join(','))}&limit=${Math.min(100,symbols.length)}`;const p=await coinbaseFetch(path,q);const rows=coinbaseProductRows(p);return symbols.map(s=>{const r=rows.find(x=>upper(x?.product_id)===s);const d=r?parseCoinbaseProduct(r,s):null;return d?{native_symbol:s,ticker:{source:d.source,last_price:d.price,price_change_24h_ratio:d.price_change_24h_ratio,high_24h:null,low_24h:null,open_24h:null,volume_24h:d.volume_24h,turnover_24h:d.turnover_24h,best_bid:null,best_ask:null,best_bid_size:null,best_ask_size:null,timestamp_ms:null,mark_price:null,index_price:null,open_interest:null,open_interest_value:null,funding_rate:null,next_funding_time_ms:null}}:null;}).filter(Boolean);}
