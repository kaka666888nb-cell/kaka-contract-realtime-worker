// Step1026.1: Coinbase EQUITY canonical-product fix on the Step1026 shared market layer.
// Coinbase equities keep the canonical Products API product_id (including opaque IDs) and use authenticated server-side CDP product/book/trade endpoints.
...
const VERSION = '650.8.15.147';
const DATA_VERSION = 10262;
...
async function fetchCoinbaseCanonicalProduct(identity){
  ...
  const exactPath=`/api/v3/brokerage/products/${encodeURIComponent(productId)}`;
  try{return await coinbaseFetch(exactPath);}
  catch(firstError){
    const listPath='/api/v3/brokerage/products';
    try{const list=await coinbaseFetch(listPath,coinbaseProductIdsQuery([productId],identity));const row=coinbaseProductRows(list).find(x=>upper(x?.product_id)===productId);if(row)return row;}
    catch{}
    throw firstError;
  }
}
async function buildCoinbase(identity){
  ...
  const productPath=`/api/v3/brokerage/products/${encodeURIComponent(productId)}`;
  const bookPath='/api/v3/brokerage/product_book';
  const tradesPath=`/api/v3/brokerage/products/${encodeURIComponent(productId)}/ticker`;
