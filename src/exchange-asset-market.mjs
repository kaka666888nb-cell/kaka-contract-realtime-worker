// Step1026.8 fix from 650.8.15.153 after real-device audit found OKX X-Perp exact detail could gateway-fail while the shared FUTURES ticker batch was healthy.
...
function coinbaseProductIdKey(v) {
  // Coinbase EQUITY product_id is an opaque canonical identifier. Do not apply the
  // crypto-symbol upper-case normalizer: live Step1018/V50 probes proved lower-case
  // 64-hex canonical ids are accepted by the exact Product endpoint.
  const s = str(v);
  if (!s || s.length > 180 || !/^[A-Za-z0-9._:\-/]+$/.test(s)) return '';
  return s;
}
function providerNativeKey(provider, v) {
  return provider === 'coinbase' ? coinbaseProductIdKey(v) : nativeKey(v);
}
function symbolList(v, provider = '') {
  const out = [];
  const seen = new Set();
...
async function jsonFetch(url, provider, { headers = {}, timeoutMs = FETCH_TIMEOUT_MS } = {}) {
  ...
  try {
    const r = await fetch(url, {
      method: 'GET',
      headers: { accept: 'application/json', 'user-agent': 'KakaWeb3/Step1026.7-AssetMarket', ...headers },
...
async function step1035StockParentJson(path, timeoutMs = 5000) {
  const port = Math.max(1, Number(process.env.PORT || 10000));
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(new Error('step1035_parent_timeout')), timeoutMs);
  try {
    const response = await fetch(`http://127.0.0.1:${port}${path}`, { headers: { accept:'application/json', 'cache-control':'no-cache', 'user-agent':'KakaWeb3-ExchangeAssets-Internal/1035' }, signal:controller.signal });
    const raw = await response.text(); let data = null; try { data = raw ? JSON.parse(raw) : null; } catch { throw new Error(`step1035_parent_invalid_json_${response.status}`); }
    if (!response.ok) throw new Error(`step1035_parent_http_${response.status}`); return data;
  } finally { clearTimeout(timer); }
}
...
async function getCoinbaseEquityMeta(productId) {
  const id = coinbaseProductIdKey(productId);
...
function isOkxXperp(identity){
  return identity?.provider==='okx' && /_XPERP-/i.test(str(identity?.nativeSymbol));
}
...
async function buildOkx(identity){
  const instType=okxInstType(identity),base='https://www.okx.com',xperp=isOkxXperp(identity);
  // Real-device Step1026 audit proved the shared FUTURES ticker list is healthy for
  // current X-Perps even when an exact detail call can fail at the gateway. Reuse
  // that official product universe for X-Perp ticker identity instead of requiring
  // the single-instrument ticker route.
  const tickerUrl=xperp
    ? `${base}/api/v5/market/tickers?instType=FUTURES`
    : `${base}/api/v5/market/ticker?instId=${encodeURIComponent(identity.nativeSymbol)}`;
  const tasks=[
    jsonFetch(tickerUrl,'okx',{timeoutMs:7000}),
    jsonFetch(`${base}/api/v5/public/instruments?instType=${encodeURIComponent(instType)}&instId=${encodeURIComponent(identity.nativeSymbol)}`,'okx',{timeoutMs:7000}),
    jsonFetch(`${base}/api/v5/market/books?instId=${encodeURIComponent(identity.nativeSymbol)}&sz=20`,'okx',{timeoutMs:4500}),
    jsonFetch(`${base}/api/v5/market/trades?instId=${encodeURIComponent(identity.nativeSymbol)}&limit=20`,'okx',{timeoutMs:4500}),
  ];
  if(instType!=='SPOT'){
    tasks.push(jsonFetch(`${base}/api/v5/public/open-interest?instType=${encodeURIComponent(instType)}&instId=${encodeURIComponent(identity.nativeSymbol)}`,'okx',{timeoutMs:4500}));
    tasks.push(jsonFetch(`${base}/api/v5/public/mark-price?instType=${encodeURIComponent(instType)}&instId=${encodeURIComponent(identity.nativeSymbol)}`,'okx',{timeoutMs:4500}));
    if(instType==='SWAP'||xperp)tasks.push(jsonFetch(`${base}/api/v5/public/funding-rate?instId=${encodeURIComponent(identity.nativeSymbol)}`,'okx',{timeoutMs:4500}));
  }
  ...
  if(instType!=='SPOT'){
    ...
    if((instType==='SWAP'||xperp)&&rs[6]?.status==='fulfilled'){
      funding=num(firstOkxValue(rs[6].value,'fundingRate'));
      nextFunding=num(firstOkxValue(rs[6].value,'fundingTime'));
    }
  }
  const anyPrimary=Boolean(ticker||rules||book||trades.length||oi!=null||mark!=null||funding!=null);
  return {
    ticker,orderbook:book,trades,rules,
    status:rules?{source:rules.source,trading_status:rules.state,rule_type:rules.rule_type,list_time_ms:rules.list_time_ms,expiry_time_ms:rules.expiry_time_ms,continuous_trading_switch_time_ms:rules.continuous_trading_switch_time_ms,premarket_switch_time_ms:rules.premarket_switch_time_ms}:null,
    trading_hours:rules?{source:rules.source,continuous_trading_switch_time_ms:rules.continuous_trading_switch_time_ms,premarket_switch_time_ms:rules.premarket_switch_time_ms,explicit_session_hours:null}:null,
    derivatives:instType==='SPOT'?null:{source:'okx_official_v5_public_derivative_market',mark_price:mark,index_price:null,open_interest:oi,funding_rate:funding,next_funding_time_ms:nextFunding},
    identity_only_partial:!anyPrimary,
    capabilities:{
      ticker:capability(ticker?'official_public':'temporarily_unavailable',xperp?'/api/v5/market/tickers?instType=FUTURES':'/api/v5/market/ticker'),
      orderbook:capability(book?'official_public':'temporarily_unavailable','/api/v5/market/books'),
      trades:capability(trades.length?'official_public':'temporarily_unavailable','/api/v5/market/trades'),
      rules:capability(rules?'official_public':'temporarily_unavailable','/api/v5/public/instruments'),
      status:capability(rules?'official_public':'temporarily_unavailable','/api/v5/public/instruments'),
      trading_hours:capability(rules?'official_partial':'temporarily_unavailable','/api/v5/public/instruments','only official published switch/list/expiry times; no synthetic session hours'),
      derivatives:instType==='SPOT'?capability('not_applicable'):capability((oi!=null||mark!=null||funding!=null)?'official_partial':'temporarily_unavailable','OKX public mark-price/open-interest/funding endpoints'),
    },
  };
}
...
async function buildGateCashStock(identity){
  // Step1035: all Gate cash-stock detail reads consume the fixed shared stock
  // catalog/session layer. User reads no longer start Gate stock requests.
  let bridge=null;
  try{bridge=await step1035StockParentJson(`/api/stock-catalog-v2/exact?provider=gate&symbol=${encodeURIComponent(identity.nativeSymbol)}`);}catch{bridge=null;}
  const shared=bridge?.row&&typeof bridge.row==='object'?bridge.row:null;
  if(!shared){
    return{ticker:null,orderbook:null,trades:[],rules:null,status:null,trading_hours:null,derivatives:null,identity_only_partial:true,identity_only_reason:'step1035_shared_gate_cash_stock_snapshot_not_ready',capabilities:{ticker:capability('unavailable_public_catalog_has_no_market_price'),orderbook:capability('not_collected_in_full_shared_catalog',null,'do not create per-user Gate stock orderbook requests'),trades:capability('unavailable_public_api'),rules:capability('temporarily_unavailable','/api/stock-catalog-v2/current'),status:capability('temporarily_unavailable','/api/stock-catalog-v2/current'),trading_hours:capability('temporarily_unavailable','/api/stock-catalog-v2/current'),derivatives:capability('not_applicable')},source_cache_status:'step1035_shared_snapshot_pending'};
  }
  const meta=shared.provider_metadata&&typeof shared.provider_metadata==='object'?shared.provider_metadata:{};
  const rules={source:'gate_official_shared_full_stock_catalog',symbol:shared.display_symbol,exchange:meta.exchange||shared.exchange_name,currency:shared.quote_asset,status:shared.trade_status||shared.status,trade_mode:shared.trade_mode,order_fill_timing:shared.order_fill_timing,price_precision:meta.price_precision??null,volume_precision:meta.volume_precision??null,quote_currency_precision:meta.quote_currency_precision??null,is_ipo:meta.is_ipo??null,ipo_price:meta.ipo_price??null,fx_rate:meta.fx_rate??null};
  const status={source:rules.source,trading_status:shared.trade_status||shared.status,trade_mode:shared.trade_mode,order_fill_timing:shared.order_fill_timing,trading_halted:shared.trading_halted};
  const hours={source:rules.source,current_session:shared.current_session,session_policy:shared.session_policy,supports_24_7:shared.supports_24_7,supports_24_5:shared.supports_24_5,trade_status:shared.trade_status};
  return{ticker:null,orderbook:null,trades:[],rules,status,trading_hours:hours,derivatives:null,capabilities:{ticker:capability('unavailable_public_catalog_has_no_market_price',null,'Gate Stock symbols API does not publish a full-market price field'),orderbook:capability('not_collected_in_full_shared_catalog',null,'kept unavailable rather than creating user-scaled upstream requests'),trades:capability('unavailable_public_api'),rules:capability('official_shared','/api/stock-catalog-v2/current'),status:capability('official_shared','/api/stock-catalog-v2/current'),trading_hours:capability('official_shared','/api/stock-catalog-v2/current'),derivatives:capability('not_applicable')},source_cache_status:'step1035_shared_stock_snapshot'};
}
...
function parseCoinbaseProduct(p, productId) {
  ...
  const id = coinbaseProductIdKey(p.product_id);
...
async function buildCoinbaseEquity(identity) {
  // Step1035: Coinbase cash-equity user reads consume only the fixed shared stock
  // collector snapshot. No per-user Coinbase request is started from this detail route.
  let bridge = null;
  try { bridge = await step1035StockParentJson(`/api/stock-catalog-v2/exact?provider=coinbase&symbol=${encodeURIComponent(identity.nativeSymbol)}`); } catch { bridge = null; }
  const shared = bridge?.row && typeof bridge.row === 'object' ? bridge.row : null;
  const sharedTicker = bridge?.ticker && typeof bridge.ticker === 'object' ? { ticker: bridge.ticker } : null;
  if (!shared) {
    return {
      ticker: sharedTicker?.ticker || null, orderbook: null, trades: [], rules: null, status: null, trading_hours: null, derivatives: null,
      identity_only_partial: true,
      identity_only_reason: 'step1035_shared_coinbase_equity_snapshot_not_ready',
      capabilities: {
        ticker: capability(sharedTicker?.ticker?.last_price != null ? 'official_shared_catalog_snapshot' : 'temporarily_unavailable', '/api/stock-catalog-v2/current'),
        orderbook: capability('unavailable_not_proven_for_equity'), trades: capability('unavailable_not_proven_for_equity'),
        rules: capability('temporarily_unavailable', '/api/stock-catalog-v2/current'), status: capability('temporarily_unavailable', '/api/stock-catalog-v2/current'),
        trading_hours: capability('temporarily_unavailable', '/api/stock-catalog-v2/current'), derivatives: capability('not_applicable'),
      },
      source_cache_status: 'step1035_shared_snapshot_pending',
    };
  }
  const meta = shared.provider_metadata && typeof shared.provider_metadata === 'object' ? shared.provider_metadata : {};
  const flags = meta.equity_trading_flags && typeof meta.equity_trading_flags === 'object' ? meta.equity_trading_flags : {};
  const rules = {
    source: 'coinbase_official_shared_full_equity_catalog_snapshot',
    product_id: shared.exchange_symbol, product_type: 'EQUITY', product_venue: shared.product_venue,
    status: shared.status, base_currency_id: shared.base_asset, quote_currency_id: shared.quote_asset,
    base_display_symbol: shared.display_symbol, quote_display_symbol: shared.quote_asset,
    display_name: shared.display_name, alias: meta.alias || null,
    base_increment: pos(meta.base_increment), quote_increment: pos(meta.quote_increment), price_increment: pos(meta.price_increment),
    base_min_size: pos(meta.base_min_size), base_max_size: pos(meta.base_max_size), quote_min_size: pos(meta.quote_min_size), quote_max_size: pos(meta.quote_max_size),
    is_disabled: bool(meta.is_disabled), view_only: bool(meta.view_only), cancel_only: bool(meta.cancel_only), limit_only: bool(meta.limit_only), post_only: bool(meta.post_only),
    ...
    reference_market_fields: {
      semantics: 'official_shared_product_snapshot_current_fields',
      price: shared.reference_price, price_change_24h_ratio: shared.reference_change_24h_ratio,
      volume_24h: shared.reference_volume_24h, turnover_24h: shared.reference_turnover_24h,
      best_bid_price: nonneg(meta.best_bid_price), best_ask_price: nonneg(meta.best_ask_price),
      high_24h: shared.reference_high_24h, low_24h: shared.reference_low_24h, mid_market_price: nonneg(meta.mid_market_price),
    },
  };
  const ticker = sharedTicker?.ticker || (shared.reference_price != null ? {
    source: 'coinbase_official_shared_full_equity_catalog_snapshot', last_price: pos(shared.reference_price), price_change_24h_ratio: num(shared.reference_change_24h_ratio),
    high_24h: pos(shared.reference_high_24h), low_24h: pos(shared.reference_low_24h), open_24h: null, volume_24h: nonneg(shared.reference_volume_24h),
    turnover_24h: nonneg(shared.reference_turnover_24h), best_bid: nonneg(meta.best_bid_price), best_ask: nonneg(meta.best_ask_price), best_bid_size: null, best_ask_size: null,
    timestamp_ms: Date.parse(shared.reference_market_fetched_at || shared.source_cached_at || '') || null, mark_price: null, index_price: null, open_interest: null,
    open_interest_value: null, funding_rate: null, next_funding_time_ms: null,
  } : null);
  ...
  const hours = { source: rules.source, current_session: eq.current_session, trading_day_info: eq.trading_day_info, recent_trading_days: eq.recent_trading_days, trading_halted_start_time: eq.trading_halted_start_time, trading_halted_end_time: eq.trading_halted_end_time, session_policy: shared.session_policy, supports_24_7: shared.supports_24_7, supports_24_5: shared.supports_24_5 };
  return { ticker, orderbook: null, trades: [], rules, status, trading_hours: hours, derivatives: null,
    capabilities: { ticker: capability(ticker ? 'official_shared_catalog_snapshot' : 'temporarily_unavailable','/api/stock-catalog-v2/current','fixed shared stock collector; user reads start zero Coinbase requests'), orderbook: capability('unavailable_not_proven_for_equity'), trades: capability('unavailable_not_proven_for_equity'), rules: capability('official_shared','/api/stock-catalog-v2/current'), status: capability('official_shared','/api/stock-catalog-v2/current'), trading_hours: capability('official_shared','/api/stock-catalog-v2/current'), derivatives: capability('not_applicable') },
    source_cache_status: 'step1035_shared_stock_snapshot' };
}
...
async function fetchBatchGate(identity,symbols){if(isGateCashStock(identity)){let sharedPayload=null;try{sharedPayload=await step1035StockParentJson(`/api/stock-catalog-v2/tickers?provider=gate&symbols=${encodeURIComponent(symbols.join(','))}`);}catch{sharedPayload=null;}const items=Array.isArray(sharedPayload?.items)?sharedPayload.items:[];return symbols.map(s=>{const row=items.find(x=>String(x?.native_symbol||'')===s);return row?{native_symbol:s,ticker:null,current_session:row.current_session??null,trade_status:row.trade_status??null,trade_mode:row.trade_mode??null,trading_halted:row.trading_halted??null,supports_24_7:row.supports_24_7??null,supports_24_5:row.supports_24_5??null,source_cache_status:'step1035_shared_gate_session'}:null;}).filter(Boolean);}const settle=gateSettle(symbols[0]||'',identity.marketType);let last=null;for(const host of ['https://api.gateio.ws/api/v4','https://fx-api.gateio.ws/api/v4']){try{const p=await jsonFetch(`${host}/futures/${settle}/tickers`,'gate');return symbols.map(s=>({native_symbol:s,ticker:parseGateFuturesTicker(p,s)})).filter(x=>x.ticker);}catch(e){last=e;}}throw last||new Error('gate_batch_tickers_failed');}
async function fetchBatchCoinbase(identity, symbols) {
  ...
  // Step1035: cash-equity list reads are zero-upstream and consume the fixed shared
  // full-equity market snapshot refreshed independently of users.
  let sharedPayload = null;
  try { sharedPayload = await step1035StockParentJson(`/api/stock-catalog-v2/tickers?provider=coinbase&symbols=${encodeURIComponent(symbols.join(','))}`); } catch { sharedPayload = null; }
  const sharedItems = Array.isArray(sharedPayload?.items) ? sharedPayload.items : [];
  return symbols.map(s => {
    const shared = sharedItems.find(x => String(x?.native_symbol || '') === s);
    return shared?.ticker ? { native_symbol:s, ticker:shared.ticker, capability:{state:'official_shared_catalog_snapshot'}, source_cache_status:'step1035_shared_stock_snapshot' } : null;
  }).filter(Boolean);
}
