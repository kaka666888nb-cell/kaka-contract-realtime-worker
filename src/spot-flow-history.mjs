...
const BUILD_PROVIDER_MAX_QUEUE = 8;
...
function acquireBuildSlot(provider, signal) {
  ...
  if (state.queue.length >= BUILD_PROVIDER_MAX_QUEUE || buildQueueTotal() >= BUILD_MAX_QUEUE) {
    ...
    state.rejected += 1;
    return Promise.reject(new Error(state.queue.length >= BUILD_PROVIDER_MAX_QUEUE ? 'spot_flow_history_provider_queue_full' : 'spot_flow_history_global_queue_full'));
  }
