...
function legacyPolicy(url) {
  ...
  // Step650.8.15.2：这三条 Binance 合约路由已分别由 WebSocket 快照或官方归档+共享REST守卫+实时桥接提供，
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    // Step650.8.15.2: all HTTP market endpoints run in the parent process so Binance
